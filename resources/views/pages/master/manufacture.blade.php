...
<div id="editModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center hidden">
    <div class="bg-white rounded-lg shadow-lg p-8 w-96 relative">
        ...
        <h2 class="text-center text-xl font-semibold mb-6">Ubah Produsen</h2>
        <form method="POST" id="edit-manufacture-form">
            @csrf
            @method('PUT')
            <div class="mb-4">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('manufacture-search').addEventListener('input', handleSearchInput);
        ...
        function fetchManufactures(page = 1) {
            currentPage = page;
            axios.get(`/api/v1/manufactures?per_page=${per_page}&page=${page}&search=${searchQuery}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            })
            .then(response => {
                console.log('Manufactures data:', response.data);
                renderManufactureTable(response.data);
        ...
        // 💡 Tambahkan ini agar fungsi fetchManufactures bisa dipanggil dari HTML onclick
        window.fetchManufactures = fetchManufactures;
        ...
        function handleCreateForm(e) {
            ...
            axios.post('/api/v1/manufactures', formData, {
                headers: {
                    'Authorization': `Bearer ${token}`,
            ...
            .then(response => {
                ...
                fetchManufactures(currentPage);
                e.target.reset();
            })
            .catch(error => {
                console.error('Failed to create manufacture:', error);
            });
        }
        ...
        function handleEditForm(e) {
            e.preventDefault();
            const formData = new FormData(e.target);
            const manufactureId = selectedId;

            axios.post(`/api/v1/manufactures/${manufactureId}`, formData, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'multipart/form-data'
                },
            ...
            .then(response => {
                console.log('Manufacture updated:', response.data);
                closeEditModal();
                fetchManufactures(currentPage);
            })
            .catch(error => {
                console.error('Failed to update manufacture:', error);
            });
        }
            ...
            data.data.data.forEach((item, index) => {
                ...
                row.innerHTML = `
                    ...
                    <td class="py-3 px-6 flex justify-center">
                        ...
                        <button type="button" class="bg-red-500 text-white text-sm px-2 py-2 rounded-lg shadow hover:bg-red-600" title="Delete" onclick="deleteManufacture(${item.id})">
                            <svg width="20" height="21" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        function renderPagination(data) {
            ...
            paginationHTML += `
                <span onclick="${currentPage === 1 ? '' : `fetchManufactures(${currentPage - 1})`}"
            ...
            links.forEach((link, index) => {
                ...
                paginationHTML += `
                    <span onclick="fetchManufactures(${link.label})"
            ...
            paginationHTML += `
                <span onclick="${currentPage === lastPage ? '' : `fetchManufactures(${currentPage + 1})`}"
                    class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium
        ...
        // Global deleteManufacture
        window.deleteManufacture = function (id) {
            if (confirm('Are you sure you want to delete this manufacture?')) {
                axios.delete(`/api/v1/manufactures/${id}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                })
                .then(response => {
                    console.log('Manufacture deleted:', response.data);
                    fetchManufactures(currentPage);
                })
                .catch(error => {
